...
realpathSync(state.engineRoot);
execFileSync(state.nodePath, [
  "--input-type=module",
  "-e",
  `const { listKnownProfileIds } = await import(${JSON.stringify(pathToFileURL(resolve(state.engineRoot, "dist", "profile.js")).href)}); if ((await listKnownProfileIds()).length < 1) process.exit(2);`,
], {
  env: { ...process.env, WORKFORGE_MCP_PROFILE_REGISTRY: state.registryPath },
  timeout: 5_000,
  stdio: "ignore",
});
console.log(`WORKFORGE_MACOS_DOCTOR_OK (${process.arch})`);
